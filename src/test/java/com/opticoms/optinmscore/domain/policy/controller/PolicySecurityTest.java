...
class PolicySecurityTest {
    ...
    private static final String CREATE_JSON = "{\"name\":\"policy-1\"}";
    ...
    void operatorRole_GET_200() throws Exception {
        ...
        when(policyService.listPolicies(eq(TENANT), any(Pageable.class))).thenReturn(Page.empty());
    ...
    void adminRole_POST_200() throws Exception {
        ...
        when(policyService.createPolicy(eq(TENANT), any(Policy.class))).thenReturn(entity);
        when(policyMapper.toResponse(any(Policy.class))).thenReturn(new PolicyResponse());
